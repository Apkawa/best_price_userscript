...
describe('jsdom lenta.com', () => {
  describe('Check page', () => {
    ...
    it('Checks main price', async () => {
      ...
      expect(
        document.querySelector('lu-product-page-info .GM-best-price'),
      ).toBeTruthy();
    });
    it('Checks multiple call', async () => {
      ...
      await initProductPage();
      equal(
        document.querySelectorAll('lu-product-page-info .GM-best-price').length, 1)
    });
    it('Checks recommends block', async () => {
      await initCatalog();
      expect(
        document.querySelector('lu-product-alternative .GM-best-price'),
      ).toBeTruthy();
    });
  });
});
